...
async def create_job(body: JobCreateRequest, background_tasks: BackgroundTasks):
    ...
    job = job_store.create_job(body.mode)
    background_tasks.add_task(_run_job, job.job_id, body)
    return job
...
async def _run_job(job_id: str, body: JobCreateRequest):
    job = job_store.get_job(job_id)
...
async def _process_company(job_id: str, company_name: str, objective: str):
    ...
    report = build_report(raw, company_name)
    return [report]
...
async def _process_geography(job_id: str, geo):
    ...
    for name in company_names:
        try:
            company_reports = await _process_company(job_id, name, geo.objective_prompt)
